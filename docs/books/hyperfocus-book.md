...
Topic:: [downtime brain](/notes/downtime-brain.md)
...
![Hyperfocus (book).webp](/books/hyperfocus-book.webp)

## 📒 Notes

### Why Focus Matters

We are always focused on something, whether towards external things or in our internal world, the question is how do we focus on what matters most to us, while avoiding [Distractions](/notes/procrastination.md).

Nowadays it's even harder to focus than before, with constant and easy distractions everywhere, we feel as if our attention is something that can't last long, so might as well try to multitask which is usually more hurtful than beneficial [Multitasking doesnt exist](/notes/multitasking-doesnt-exist.md)
...
Most of the time our attention is on "autopilot mode" [defaults](/notes/defaults.md), which is convenient when we are doing simple things that requires little effort, but if we are on autopilot mode for too long we can lose control over our attention, being unable to use and direct it towards the right targets. Our most productive, creative effort comes from intentional use of attention [mindfulness](/notes/intentionality.md).

The problem is magnified once we realize how much the environment has control over our attention and is hardly unbiased [Attention Economy](/notes/attention-economy.md).

**The most stimulating and attractive things will rarely be the most important ones** [streetlight effect](/notes/streetlight-effect.md)
...
2. **Distracting work** - things that we [rationalize](/notes/rationalization.md) are important but in the end are only distractions
...
That doesn't mean you can't reach a point of [overloading](/notes/overloading.md). By having too many tasks, we are getting overwhelmed and start to forget what we are doing, or lose focus quickly. The answer is to limit your inputs [less is more](/notes/addition-by-subtraction.md). By reducing our cognitive load, we can focus more on what's left.

Limiting is not the same as juggling between different tasks. When we switch between unfinished tasks, our mind continues to think about the previous task, thus limiting the resources we have for the task at hand. The quicker we switch, the worse we get [task switching](/notes/task-switching.md).
...
Hyperfocus is synonymous with [flow](/notes/flow.md), a sense of oneness of you and the task, a harmonious combination of the internal and external world [Extended Mind](/notes/extended-mind.md). To be hyperfocus, we need:
...
Attention is nothing without [Intention](/notes/intention.md), we need to choose what to focus on, which requires [Prioritization](/notes/triage.md). Think which actions are beneficial while considering also the long term [Domino Effect](/notes/ripple-effect.md). Decide in advance what are you going to focus on today so that you'll be less distracted by new information later on. Check yourself once in a while that you are focusing on what you set out to do.

Also, remember to be [clear](/notes/clarity.md) with your intentions, the more specific the better. Don't overcommit, [start small](/notes/start-small.md) and try 15 minute blocks with breaks in between [pomoduro technique](/notes/pomoduro-technique.md)
...
For the distractions we can't control, we just have to [accept](/notes/acceptance.md) them, be mindful during those moments to get the most out of them, and then get back to work. For the ones we can control, we need to limit as much as possible.

Using [commitment devices](/notes/commitment-devices.md) to block off notifications, [task batching](/notes/task-batching.md) to check emails and other distractions only once or twice a day, and stay away from your phone as much as possible, especially if it starts to interrupt your sleep.

For example, put your phone on airplane mode, delete apps, put it in a different room, put noise canceling headphones and notify others on when you will be available again. It's all about controlling which [Cue](/notes/priming.md) are around you and could derail you.

In terms of planning and managing tasks, remember that [our mind is a processor and not a warehouse](/notes/our-mind-is-a-processor-and-not-a-warehouse.md), it would help reduce your cognitive load if you externalize the tasks into a list or an app [Effort Storing](/notes/effort-storing.md).
...
First, hyperfocusing requires just the right amount of things that try to fill up our attention. If we find ourselves [bored](/notes/boredom.md) often then we lack enough meaningful complex projects in our lives, and the reverse is true if we feel [burnout](/notes/burnout.md) even after trying to narrow down our focus.

Aside from managing our attention, we can increase the amount of attention we can spare. [Meditation](/notes/meditation.md) is one of the best things we can do to improve our memory and focus capacity.
...
While we often need to concentrate, we also need to daydream. This mode allows us to explore different ideas and test them out in our brain [simulations](/notes/simulations.md). We think about the future and imagine possible scenarios based on what we can do, and ruminate on the past thinking what we could have done better. This type of thinking is the peak of human [creativity](/notes/ideation.md). We can compare Scatterfocus and hyperfocus to [exploration vs exploitation](/notes/exploration-vs-exploitation.md), one is useful in generating new ideas and the other in implementing them. Usually Scatterfocus doesn't come when we try to force it out, but rather when we let go [The law of reverse effect](/notes/the-law-of-reverse-effect.md), for example having creative thoughts when we shower or talk a walk. Therefore in order to have moments of Scatterfocus we need to generate moments where we let our mind wander, rather than try to control it to where it wanders off [Wu wei](/notes/wu-wei.md).
...
[Rest](/notes/rest.md) is essential for our productivity. After all, it is only with energy that we are capable of being productive. Therefore [sleep](/notes/sleep.md) and rest are a [Multiplier](/notes/multiplier.md) of productivity, and an investment worth making. Besides eight hours of sleep, you need to take a brake every 90 minutes for 15 minutes to maintain optimal energy levels.
...
We can intentionally leave some tasks unfinished so that our brain would process them in the background over time. [Attention Residue](/notes/attention-residue.md)

#### Collecting Dots

It's all about [Connection Making](/notes/coherentism.md). This is the source of our productive output. The more we are able to connect different dots, the easier it is to group them together and hold them in our memory [chunking](/notes/chunking.md). For example it's easier to remember words or sentences than individual letters or syllables. Having the knowledge of language helps us make larger chunks.

Not all information is born equal. Valuable information is an actionable one [pragmatism](/notes/pragmatism.md) that improves your [intuition](/notes/intuition.md). Look for information within and outside of your expertise [Range](/notes/breath-beats-depth.md), and in general follow your [curiosity](/notes/curiosity.md). It's hard to learn all the time, so try to combine it with entertaining content but don't consume just mind numbing trash.

Evaluate the content before and during consuming it [reading methods](/notes/reading-methods.md), and don't by shy to say no, it's better to stop something pointless in the middle than pushing yourself to finish it [saying no](/notes/saying-no.md).
