...
3. **Reprogram Your Environment** - redesign your surroundings and social circle to make desirable defaults automatic (surround yourself with people who exhibit good defaults).
4. **Practice Self-Accountability Daily** - when outcomes go wrong, write what you can do next to improve the situation instead of blaming external factors.
5. **Build Self-Knowledge** - document your limits, skills, recurring mistakes, and emotional triggers so you can avoid bad matches and improve decision-making.
6. **Create Safeguards Against Weaknesses** - implement prevention, automatic rules, added friction, checklists, or perspective shifts to block impulsive or harmful actions.
7. **Define Problems Clearly Before Solving** - for each issue, write (a) the goal you want and (b) the obstacles standing in the way, and avoid jumping straight to solutions.
8. **Explore Solutions Systematically** - run pre-mortems, apply second-level thinking (“and then what?”), avoid binary choices, and consider opportunity costs for each option.
...
Through acknowledging our freedom to react, we can reshape our defaults, which will in time create a positive momentum that will work in our favor. This is true strength.
