---
UUID: 20231231070530
Created: '2023-12-31 07:05'
Modified: '2026-09-02 16:25'
tags: []
excalidraw-plugin: parsed
excalidraw-autoexport: png
excalidraw-open-md: true
Version: 5
Image: '[[Identity is shaped by and shapes our social interactions.png]]'
ImageText: >-
  A venn diagram of overlapping circles, one shows friendship, one shows family,
  one shows society (a symbol of an institution) and one shows a globe. In the
  center there is a person, representing the self
Description: >-
  A concept that asserts individual identity is shaped by social interactions
  and environments, emphasizing that personal beliefs, values, and experiences
  are influenced by relationships with others.
Topic:
  - '[[Identity Structure]]'
Origin: '[[Selfless (book)]]'
URL: null
aliases:
  - Interpersonal Identity
draft: false
SiteProcssed: true
---

# Interpersonal Identity

## Notes

### Claim
Interpersonal Identity means that our identity isn't set in isolation. We are not truly ever individuals, because we are born into a social world [Human is a social being](/notes/human-is-a-social-being.md). We are shaped by our [Social environment shapes behavior and identity through norms and conformity](/notes/social-environment-shapes-behavior-and-identity-through-norms-and-conformity.md), and if we are impactful enough, we get to shape it back. The way we see the world [Subjective perception filters how we see external reality](/notes/subjective-perception-filters-how-we-see-external-reality.md), our beliefs and [Core values guide authentic identity and moral action](/notes/core-values-guide-authentic-identity-and-moral-action.md), our [happiness](/notes/happiness-is-shared.md), are all affected by our interactions and experiences with others.

### Explanation
We often [imitate](/notes/imitation-internalizes-others-behaviors-and-mental-models-at-varying-depths.md) the behaviors of others and make it a part of our identity. The say that close friends and partners tend to act alike as time goes by. By doing so, it is as if we hold a collection of identities, of other people we wish to imitate within ourselves, forming together our unique identity [A person is a community](/notes/a-person-is-a-community.md).

A more extreme notion claims that our identity is a complete [Shared beliefs make constructs real and true](/notes/shared-beliefs-make-constructs-real-and-true.md). That the different roles we assume (whether by choice or by force) such as parent, friend, partner, even racial and gender group are all assigned by others. We can't be friends with people who don't love us back. We can't be a parent to a random kid of the street, there has to be a connection to base it on. The way others perceive who we are, is who we are.

### Why it matters

### Examples
Imagine yourself being born to different parents, perhaps in a different time, in a different state. Would you really say that you would be the same person? That your preferences, dreams and viewpoints would be exactly the same? I think that the obvious answer is not [Nature vs nurture](/notes/nature-vs-nurture.md). We owe much, if not all, of our identity to others, to humanity. Even something small as a dream to be an astronaut, only exists because others have invented space travel, and you heard stories about it from others who did it.

### Supporters
It could be argued that our innate tendency to [mirror](/notes/mirroring-copies-others-emotional-states-and-body-language-to-build-empathy.md) and [empathize](/notes/empathy-means-understanding-and-acting-on-anothers-perspective.md) with others is a result of our mixed identity, a proof that the individual level is never the right scope for understanding a person, that we always have to look at the network associated with them [Behavior reveals true preferences](/notes/behavior-reveals-true-preferences.md).

### Opposers

Not everyone would grant that our identity is this porous. [Individualist](/notes/the-individual-is-the-smallest-unit-of-moral-worth.md) views push back, insisting that beneath all the social shaping there's still an independent core to who we are, one that exists prior to and separate from our relationships.

### Open questions
## Visual

![Identity is shaped by and shapes our social interactions](/notes/identity-is-shaped-by-and-shapes-our-social-interactions.webp)
